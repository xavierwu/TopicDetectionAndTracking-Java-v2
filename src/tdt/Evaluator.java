...
public class Evaluator {
	 ...
	 * @return normCdet, 0~1
	 */
	protected static double doEvaluation(Vector<Story> corpus, Vector<Story> actualFirstStories,
	...
	private static ArrayList<Double> doEvaluation_v3(Vector<Story> corpus, Vector<Story> actualFirstStories,
		...
		double PFa = (firstStories.size() - numOfHits) / Double.valueOf(firstStories.size());
		double CFa = 0.1;
		double Pnon_target = 1 - Ptarget;
		arr.add(PFa);

		double Cdet = PMiss * (CMiss * Ptarget) + PFa * (CFa * Pnon_target);
		double norm_mini = (CMiss * Ptarget < CFa * Pnon_target) ? (CMiss * Ptarget) : (CFa * Pnon_target);
	 ...
	 * @return
	 */
	private static double doEvaluation_v2(Vector<Story> corpus, Vector<Story> actualFirstStories,
			Vector<Story> firstStories) {
		int numOfHits = 0;
		for (int i = 0; i < actualFirstStories.size(); ++i) {
			for (int j = 0; j < firstStories.size(); ++j) {
	...
	private static double doEvaluation_v1(Vector<Story> corpus, Vector<Story> actualFirstStories,
			Vector<Story> firstStories) {
		...
		for (int j = 0; j < actualFirstStories.size(); j++) {
			answer[j] = actualFirstStories.get(j).getTimeStamp().substring(0, 8);
		}
		Arrays.sort(answer);
		...
		for (int i = 0; i < firstStories.size(); i++) {
			...
			for (int j = 0; j < actualFirstStories.size(); j++) {
				String str4 = actualFirstStories.get(j).getTimeStamp().substring(0, 8);
